...
- **A failed guest `assert` surfaces as a write-once memory conflict**, not an assertion message, but it names the function and source line: `write-once conflict at cell 34: had ..., new ... at pc ... (in verify_sub (line 2906))`, as does every other `ExecError` (a failed range check, a wild `DEREF`). Parse and lowering errors carry a line too. Reach for `DBG_DISASM` only when the line is not enough, or when the pc lands in a fill block, which has no source line by construction.
